...
def main():
    ...
    for i in range(n):
        ...
        # ! back-retrieval
            ...
            query = [data[i]["query"], data[i]["answer"]]
            query = " ".join(query)
        ...
        # ! for back-retrieval: originalCTX_resp_bwd#_fwd#
            ...
            bwd_ctx = [item for pair in zip(curr_turn_dt['history_query'], curr_turn_dt['history_answer'])
                         for item in pair] + [curr_turn_dt['query']]
            ...
            fwd_ctx = [item for pair in zip(fwd_turn_dt['history_query'], fwd_turn_dt['history_answer'])
                         for item in pair] + [fwd_turn_dt['query']] + [fwd_turn_dt['answer']]
    ...
    # pyserini search
    searcher = SimpleSearcher(args.index_dir_path) # LuceneSearcher(args.index_dir_path)
    ...
    with open(oj(args.output_dir_path, output_f), "w") as f:
        for qid in qid_list:
            for i, item in enumerate(hits[qid]):
                item_docid = '/'.join(item.docid.split("/")[6:])
                # item_docid = item_docid.replace(" ", "")
                # f.write("{} {} {} {} {} {} {}".format(qid,
                #                                 "Q0",
                #                                 item_docid,
                #                                 i+1,
...
def get_args():
    ...
    parser.add_argument("--index_dir_path", type=str, default="/data2/../nlp_data/kisti/preprocessed/pyserini_index")
    parser.add_argument("--output_dir_path", type=str, default="/data2/../nlp_data/convgqr/bm25/kisti")
    parser.add_argument("--output_f", type=str, default=None)
    parser.add_argument("--gold_qrel_file_path", type=str, default="/data2/../nlp_data/kisti/dev_gold.trec")
